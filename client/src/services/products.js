...
function getCategory(id) {
    return fetch(`${process.env.REACT_APP_SERVERLESS_URL}/api/products?category_id=${id}&limit=30`, {
        headers: {
            'Content-type': 'application/json',
        },
    })
...
function getByBrands(id, brand) {
    return fetch(`${process.env.REACT_APP_SERVERLESS_URL}/api/products?category_id=${id}&limit=30&brand=${brand}`, {
        headers: {
            'Content-type': 'application/json',
        },
    })
